...
<%
	...
	int alarm = (request.getAttribute("alarm") != null) ? (Integer) request.getAttribute("alarm") : 0; // 기본값 설정
	...
	ArrayList<MyWorkspaceDto> myWorkspaceMemberList = (ArrayList<MyWorkspaceDto>)request.getAttribute("myWorkspaceMemberList");
...
<head>
<meta charset="UTF-8" />
<title>페이지 - 내 작업 공간</title>
<link rel="stylesheet" href="css/Main_Common.css" />
<link rel="stylesheet" href="css/asana_my_workspace.css" />
...
<script>
	...
	/* 사이드바 목록 선택 시 어디로 이동 할지 *//* 사이드바 목록 선택 시 어디로 이동 할지 *//* 사이드바 목록 선택 시 어디로 이동 할지 */
		...
		/* 사이드바 목록 선택 시 어디로 이동 할지 *//* 사이드바 목록 선택 시 어디로 이동 할지 *//* 사이드바 목록 선택 시 어디로 이동 할지 */
		/* 내작업 공간 화면 프로젝트 리스트의 ***클릭시 */
		$(".Icon16.project_list_etc").click(function() {
			$(".project_etc_popup_list").removeClass("on");
			$(this).parents().eq(3).find(".project_etc_popup_list").toggleClass("on");
			event.stopPropagation();
		});
		$("body").click(function(){
			$(".project_etc_popup_list").removeClass("on");
		});
		/* 프로젝트 etc 팝업창 요소 클릭시 *//* 프로젝트 etc 팝업창 요소 클릭시 *//* 프로젝트 etc 팝업창 요소 클릭시 *//* 프로젝트 etc 팝업창 요소 클릭시 *//* 프로젝트 etc 팝업창 요소 클릭시 *//* 프로젝트 etc 팝업창 요소 클릭시 *//* 프로젝트 etc 팝업창 요소 클릭시 */
		 $(".project_etc_popup_list_content").click(function(){
			let name = $(this).find("div:first-child").text().trim();  // 별표 표시에 추가, 상단에 고정
			let name2 = $(this).find("div:nth-child(2)").text().trim();  // 별표 표시에서 제거, PIN 제거
			let projectName = $(this).parent().prev().find(".dtos_project_get_name").text(); //프로젝트 이름 따오기
			let projectIdx = $(this).parent().attr('project_idx'); //프로젝트idx 따오기	
			let favoritesProjectIdx = $(".sidebar_item.ajax."+projectIdx).attr("project_idx");//즐겨찾기 돼있다면 프로젝트idx 따오기 
			$(this).parent().removeClass("on");
			if(name==="공유...") {
				alert("공유...");
			} else if(name==="프로젝트 삭제") {
				$.ajax({
					type:'post',
					data:{'loginMemberIdx':${loginMemberIdx},
						'projectIdx':projectIdx},
					url:'AjaxProjectDeleteServlet',
					success:function(data){
						/* 일반 사이드바 칸 */
						$(".sidebar_item[project_idx='" + projectIdx + "']").remove();
						/* 즐겨찾기 칸 */
						$(".sidebar_item.ajax[project_idx='" + projectIdx + "']").remove();
						/* 팝업창 닫기 */
						$(".project_etc_popup_list."+projectIdx).parent().remove();
						alert(projectIdx + "번 프로젝트가 삭제되었습니다");
					},
					error:function(r,s,e){alert("error");}
				});
			}
			if(name==="별표 표시에 추가" && $(this).find("div:first-child").css('display')=='flex') {
				$(this).find(".project_etc_add_star").css('display','none');
				$(this).find(".project_etc_remove_star").css('display','flex');
				$.ajax({
					type:'post',
					data:{'loginMemberIdx':${loginMemberIdx},
							'projectIdx':projectIdx},
					url:'AjaxAddProjectFavoritesServlet',
					success:function(data){
						$(".category.ajax").show();
						let str =
					        '<div class="sidebar_item ajax ' + projectIdx + '" project_idx="' + projectIdx + '">' +
					        ...
					        "</svg>" + '<span class="ajax_dtos_project_favorites_name">' + projectName + "</span>" + "</div>";
					    $(".category.with_plus").before(str);
					},
					error:function(r,s,e){alert("error");}
				});
			} else if(name2==="별표 표시에서 제거" && $(this).find("div:nth-child(2)").css('display')=='flex') {
				$(this).find(".project_etc_add_star").css('display','flex');
				$(this).find(".project_etc_remove_star").css('display','none');
				/* 즐겨찾기 칸 */
				$(".sidebar_item.ajax[project_idx='" + projectIdx + "']").remove();
				$.ajax({
					type:'post',
					data:{'loginMemberIdx':${loginMemberIdx},
							'projectIdx':projectIdx},
					url:'AjaxRemoveProjectFavoritesServlet',
					success:function(data){
						if(data.cnt==0){
			...
			if(name==="상단에 고정" && $(this).find("div:first-child").css('display')=='flex') {
				$(this).find(".project_etc_add_fix").css('display','none');
				$(this).find(".project_etc_remove_fix").css('display','flex');
				alert(projectIdx + "번 프로젝트 상단에 고정함");
			} else if(name2==="PIN 제거" && $(this).find("div:nth-child(2)").css('display')=='flex') {
				$(this).find(".project_etc_add_fix").css('display','flex');
				$(this).find(".project_etc_remove_fix").css('display','none');
				alert(projectIdx + "번 프로젝트 PIN 제거함");
			} 
		});
		 /* 프로젝트 etc 팝업창 요소 클릭시 *//* 프로젝트 etc 팝업창 요소 클릭시 *//* 프로젝트 etc 팝업창 요소 클릭시 *//* 프로젝트 etc 팝업창 요소 클릭시 *//* 프로젝트 etc 팝업창 요소 클릭시 */
		/* 내작업공간 화면 프로젝트 생성 클릭 */
		$(".create-button").click(function() {
			location.href="Controller?command=create_project&member_idx=${loginMemberIdx}";
		});
		$(".create-button-small").click(function() {
			location.href="Controller?command=create_project&member_idx=${loginMemberIdx}";
		});
		/* 내작업공간 멤버 클릭시 */
		$(".user-list-in").click(function() {
			let clickMemberIdx = $(this).attr("memberIdx");
			let loginMemberIdx = ${loginMemberIdx};
			let name = $(this).text().trim();
			if (name === "초대") {
				$(".popup_invite_whole-div").css('display', 'flex');
			}else if(clickMemberIdx!=loginMemberIdx){
				location.href="Controller?command=other_profile&member_idx="+clickMemberIdx;
			}else if(clickMemberIdx==loginMemberIdx){
				location.href="Controller?command=profile_page&member_idx="+clickMemberIdx;
			}
		});
		/* 내작업공간 프로젝트 박스 프로젝트 클릭시 */
		$(".under-list").click(function() {
			let name = $(this).find("div:first-child > div:nth-child(2) > div:first-child").text().trim();
			let projectIdx = $(this).next().attr("project_idx");
			location.href="Controller?command=main_inventory&member_idx=${loginMemberIdx}&project_idx="+projectIdx;
		});
		/* 마우스 대면 별 테두리 켜짐 */
		...
		$(".star-center").click(function() {
			$(".sidebar_item.with_gt.ajax").remove();
			if($(this).hasClass("on")) {
				$(this).removeClass("on");
				let cnt = ${favoritesCount};
				dollar_this = $(this);
			 	$.ajax({
					type:'post',
					data:{'loginMemberIdx':${loginMemberIdx}},
					url:'removeFavoritesWorkspaceServlet',
					success:function(data){
						if(data.cnt==0){
							$(".category.ajax").hide();   // hide();
						}else {
				...
				$.ajax({
					type:'post',
					data:{'loginMemberIdx':${loginMemberIdx}},
					url:'addFavoritesWorkspaceServlet',
					success:function(data){
						$(".category.ajax").show();   //show();
						$(".sidebar_item.with_gt.ajax").show();
						let str = `<div class="sidebar_item with_gt ajax">
		...
		/* 파란 초대 버튼 클릭 */
		$(".invite-blue-button").click(function() {
			$(".popup_invite_whole-div").css('display', 'flex');
			$(".create_something_popup.on").removeClass("on");
		});
	});
	$(function(){
		<%for(ProjectDto list : pDtoFavoritesList){%>
			$(".project_etc_popup_list.<%=list.getProjectIdx()%>").find(".project_etc_add_star").css('display','none');
			$(".project_etc_popup_list.<%=list.getProjectIdx()%>").find(".project_etc_remove_star").css('display','flex');
		<%}%>
	});
</script>
</head>
...
<!-- 프로필셋팅 팝업창 --><!-- 프로필셋팅 팝업창 --><!-- 프로필셋팅 팝업창 --><!-- 프로필셋팅 팝업창 --><!-- 프로필셋팅 팝업창 -->
	...
	<div id="header">
		...
		<div id="me_button">
			...
			<span class="memberIdx" style="display:hidden;">${loginMemberIdx}</span>
		</div>
	...
	<div id="content">
		<div id="sidebar" class="fl">
			...
			<% for(ProjectDto dtos : pDtoFavoritesList){ %>
				<div class="sidebar_item ajax" project_idx="<%=dtos.getProjectIdx()%>">
					<svg class="MiniIcon--large MiniIcon ColorFillIcon ColorFillIcon--colorAqua SmallSquircleMiniIcon" viewBox="0 0 24 24" aria-hidden="true" focusable="false">
			...
			<% for(MemberDto dtos : mDaoFavoritesList){ %>
				<div class="sidebar_item ajax" member_idx="<%=dtos.getMemberIdx()%>">
			...
			<!------------------------- 즐겨찾기 추가된 프로젝트 & 포트폴리오 & 멤버 ------------------------------->
				...
				<div class="sidebar_item">
					<div>
						<svg class="Icon ButtonThemeablePresentation-leftIcon SidebarInvite-icon EmailIcon" viewBox="0 0 32 32" aria-hidden="true" focusable="false">
		...
		<div id="main" class="fl">
			<div class="main">
				<div class="main-top">
					<div class="top-left">
						<div class="circle-small">내</div>
						<div class="font20">내 작업 공간</div>
						<% if(favoritesWorkspaceCount>0){ %>
							<div class="star-center on" style="width: 25px; height: 25px;">
								<svg class="icon20-star on" viewBox="0 0 32 32" aria-hidden="true" focusable="false" style="fill: gray;">
					...
					<div class="top-right">
						...
						<div class="center">
							<button class="invite-blue-button">
								<svg class="UsersFillMiniIcon" viewBox="0 0 24 24" aria-hidden="true" focusable="false">
				...
				<div class="title">
					<div class="title-left">
						<div class="title-circle">내</div>
						<div class="title-text">내 작업 공간</div>
						<div>
							<input class="title-comment" type="text" placeholder="클릭하여 팀 설명 추가..." />
						</div>
					</div>
					<div>
						<div class="Create-project">
							<button class="create-button">새 팀 프로젝트</button>
						</div>
					</div>
				</div>
				<div class="content">
					<div class="user">
						<div>
							<div class="project-in">
								<div>
									<span>멤버(${memberLength}명)</span> 
									<span> 
										<select style="margin-left: 15px;" class="select">
											<option>자주 협업하는 협업 참여자</option>
											<option>최근 협업한 협업 참여자</option>
											<option>별표 표시된 협업 참여자</option>
										</select>
									</span>
								</div>
								<div role="button" aria-label="동작" tabindex="0"
									class="Icon16 member_box_etc">
								</div>
							</div>
						</div>
						<div class="user-list">
							<div class="user-list-in">
							...
							<% for(MyWorkspaceDto dto : myWorkspaceMemberList){
								String myprofileImg = dto.getMemberDto().getProfileImg();
								String myNickname = dto.getMemberDto().getNickname();
								Integer member = dto.getMemberDto().getMemberIdx(); %>
								<!-- 멤버1 -->
								<div class="user-list-in" memberIdx="<%=member%>">
									<div> 
										<img class="img" src="img/<%=myprofileImg%>">
									</div>
									<%=myNickname %>
								</div>
							<% } %>
					...
					<div class="content-left">
						<div class="left-top">
							<div class="top-title">
								<div class="font20">프로젝트</div>
								<div class="top-title-right">
									<div>
										<button class="create-button-small">새프로젝트</button>
									</div>
								</div>
							</div>
							<div class="top-name">
								<div class="">이름</div>
								<div class="margin180">멤버</div>
							</div>
						</div>
						<div class="left-under">
						<% for(ProjectDto dtos : pDtoList){ %>
						<div>
							<div class="under-list" project_idx="<%=dtos.getProjectIdx()%>">
								<div class="project-list">
									<div style="background-color: #4573D2; margin-right: 15px; display: flex; align-items: center; justify-content: center; width: 48px; height: 48px; border-radius: 10px;">
										<svg class="MiniIcon24" viewBox="0 0 24 24" title="list" services="[object Object]">
									...
									<div>
										<div class="dtos_project_get_name"><%=dtos.getName()%></div>
										<div style="font-size: 13px; color: green;">참여함</div>
									</div>
								</div>
								<div class="project-list2">
									...
									<div>
										<svg class="Icon16 project_list_etc" viewBox="0 0 32 32" aria-hidden="true" focusable="false">
										<path d="M16,13c1.7,0,3,1.3,3,3s-1.3,3-3,3s-3-1.3-3-3S14.3,13,16,13z M3,13c1.7,0,3,1.3,3,3s-1.3,3-3,3s-3-1.3-3-3S1.3,13,3,13z M29,13c1.7,0,3,1.3,3,3s-1.3,3-3,3s-3-1.3-3-3S27.3,13,29,13z"></path></svg>
									</div>
								</div>
							</div>
							<!------------------------------- 프로젝트 기타 설정 팝업창 ----------------------------------------------->
							<div class="project_etc_popup_list <%=dtos.getProjectIdx()%>" project_idx="<%=dtos.getProjectIdx()%>">
								<div class="project_etc_popup_list_content">
									<div class="project_etc_share">
										<svg class="project_etc_share_icon" viewBox="0 0 32 32" aria-hidden="true" focusable="false">
								...
								<!--------------------- 즐겨팢기 상태 유무에 따라 추가인지 제거인지 나눠져있어야함 ------------------------>
								<div class="project_etc_popup_list_content">
									<div class="project_etc_add_star">
										<svg class="project_etc_add_star_icon" viewBox="0 0 32 32" aria-hidden="true" focusable="false">
										<path d="M.608 11.301a2.161 2.161 0 0 0 .538 2.224l5.867 5.78-1.386 8.164c-.14.824.192 1.637.868 2.124a2.133 2.133 0 0 0 2.262.155l7.241-3.848 7.24 3.848a2.133 2.133 0 0 0 2.263-.155 2.155 2.155 0 0 0 .868-2.124l-1.386-8.164 5.867-5.78c.59-.582.797-1.434.538-2.224a2.146 2.146 0 0 0-1.734-1.466l-8.1-1.19-3.623-7.42A2.148 2.148 0 0 0 15.998 0c-.828 0-1.57.476-1.935 1.223L10.44 8.645l-8.1 1.19A2.145 2.145 0 0 0 .606 11.3h.002Zm2.025.512 9.143-1.342 4.086-8.37c.012-.023.044-.088.137-.088.093 0 .124.064.137.089l4.086 8.369 9.143 1.342c.023.003.093.014.125.11a.163.163 0 0 1-.042.178l-6.609 6.511 1.56 9.192a.16.16 0 0 1-.065.169c-.074.054-.129.023-.152.01l-8.181-4.346-8.181 4.347c-.022.013-.08.04-.152-.011a.159.159 0 0 1-.065-.17l1.56-9.19-6.61-6.512a.163.163 0 0 1-.041-.178c.032-.096.102-.106.125-.11h-.004Z"></path></svg>
										별표 표시에 추가
									</div>
									<div class="project_etc_remove_star">
										<svg class="project_etc_add_star_icon" viewBox="0 0 32 32" aria-hidden="true" focusable="false">
										<path d="M.608 11.301a2.161 2.161 0 0 0 .538 2.224l5.867 5.78-1.386 8.164c-.14.824.192 1.637.868 2.124a2.133 2.133 0 0 0 2.262.155l7.241-3.848 7.24 3.848a2.133 2.133 0 0 0 2.263-.155 2.155 2.155 0 0 0 .868-2.124l-1.386-8.164 5.867-5.78c.59-.582.797-1.434.538-2.224a2.146 2.146 0 0 0-1.734-1.466l-8.1-1.19-3.623-7.42A2.148 2.148 0 0 0 15.998 0c-.828 0-1.57.476-1.935 1.223L10.44 8.645l-8.1 1.19A2.145 2.145 0 0 0 .606 11.3h.002Zm2.025.512 9.143-1.342 4.086-8.37c.012-.023.044-.088.137-.088.093 0 .124.064.137.089l4.086 8.369 9.143 1.342c.023.003.093.014.125.11a.163.163 0 0 1-.042.178l-6.609 6.511 1.56 9.192a.16.16 0 0 1-.065.169c-.074.054-.129.023-.152.01l-8.181-4.346-8.181 4.347c-.022.013-.08.04-.152-.011a.159.159 0 0 1-.065-.17l1.56-9.19-6.61-6.512a.163.163 0 0 1-.041-.178c.032-.096.102-.106.125-.11h-.004Z"></path></svg>
										별표 표시에서 제거
									</div>
								...
								<!--------------------- 즐겨팢기 상태 유무에 따라 추가인지 제거인지 나눠져있어야함 ------------------------>
								<div class="project_etc_popup_list_content">
									<div class="project_etc_add_fix">
										<svg class="project_etc_add_fix_icon" viewBox="0 0 32 32" aria-hidden="true" focusable="false">
										<path d="M.292 31.707a.997.997 0 0 0 1.414 0l8.784-8.784 6.227 6.228a3.015 3.015 0 0 0 2.436.87 2.91 2.91 0 0 0 2.149-1.276 12.996 12.996 0 0 0 2.146-8.28l7.137-7.137a3.971 3.971 0 0 0 1.172-2.828 3.971 3.971 0 0 0-1.172-2.828l-6.258-6.258a4.004 4.004 0 0 0-5.656 0L11.534 8.55a13.01 13.01 0 0 0-8.28 2.145 2.914 2.914 0 0 0-1.276 2.15 3.014 3.014 0 0 0 .87 2.435l6.229 6.228-8.784 8.784a.999.999 0 0 0-.001 1.414Zm3.97-17.841a1.02 1.02 0 0 1-.294-.826.92.92 0 0 1 .4-.684 10.977 10.977 0 0 1 6.111-1.85c.436 0 .873.026 1.31.078l.48.057 7.815-7.814a2.003 2.003 0 0 1 2.828 0l6.258 6.258c.378.378.585.88.585 1.414 0 .534-.207 1.036-.585 1.414l-7.814 7.814.056.482a10.993 10.993 0 0 1-1.77 7.421.915.915 0 0 1-.684.4 1.03 1.03 0 0 1-.826-.295l-13.87-13.87Z"></path></svg>
										상단에 고정
									</div>
									<div class="project_etc_remove_fix">
										<svg class="project_etc_add_fix_icon" viewBox="0 0 32 32" aria-hidden="true" focusable="false">
										<path d="M.292 31.707a.997.997 0 0 0 1.414 0l8.784-8.784 6.227 6.228a3.015 3.015 0 0 0 2.436.87 2.91 2.91 0 0 0 2.149-1.276 12.996 12.996 0 0 0 2.146-8.28l7.137-7.137a3.971 3.971 0 0 0 1.172-2.828 3.971 3.971 0 0 0-1.172-2.828l-6.258-6.258a4.004 4.004 0 0 0-5.656 0L11.534 8.55a13.01 13.01 0 0 0-8.28 2.145 2.914 2.914 0 0 0-1.276 2.15 3.014 3.014 0 0 0 .87 2.435l6.229 6.228-8.784 8.784a.999.999 0 0 0-.001 1.414Zm3.97-17.841a1.02 1.02 0 0 1-.294-.826.92.92 0 0 1 .4-.684 10.977 10.977 0 0 1 6.111-1.85c.436 0 .873.026 1.31.078l.48.057 7.815-7.814a2.003 2.003 0 0 1 2.828 0l6.258 6.258c.378.378.585.88.585 1.414 0 .534-.207 1.036-.585 1.414l-7.814 7.814.056.482a10.993 10.993 0 0 1-1.77 7.421.915.915 0 0 1-.684.4 1.03 1.03 0 0 1-.826-.295l-13.87-13.87Z"></path></svg>
										PIN 제거
									</div>
								...
								<div class="project_etc_popup_list_content">
									<div class="project_etc_delete">프로젝트 삭제</div>
								</div>
							</div>
							<!------------------------------ 프로젝트 기타 설정 팝업창 --------------------------------->
						</div>
