...
<%
	...
	LookMemoDto lookMemoDto = (LookMemoDto)request.getAttribute("lookMemoDto");
%>
<html>
<head>
<meta charset="UTF-8" />
...
<link rel="stylesheet" href="css/asana_memo.css"/>
...
<script>
	...
	webSocket.onmessage = function(e){
		...
		if(e.data.startsWith("title:///")) {
			let new_str = e.data.replace("title:///","").split("///")[1];
			$(".title").val(new_str);
		}
		else if(e.data.startsWith("content:///")) {
			let new_str = e.data.replace("content:///","").split("///")[1];
			$(".content").val(new_str);
		}
	}
	...
	$(function(){
		...
		$(".title").keyup(function(e) {
			...
			// ------> "title:///메모idx///타이틀문자열"
			let memo_idx = $(this).parents(".main").attr("memo_idx");
			//alert("memo_idx : " +  memo_idx);
			// ------> "title:///메모idx///타이틀문자열"
			webSocket.send("title:///" + memo_idx + "///" + $(".title").val());
		});
		$(".content").keyup(function(e) {
			...
			//alert("memo_idx : " +  memo_idx);
			// -------> "content:///메모idx///컨텐트문자열"
			webSocket.send("content:///" + memo_idx + "///" + $(".content").val());
		});
		
		/* 메모 제목 입력 시  */
		$(".title").keyup(function(event) {
			
		    if (event.key === "Enter" || event.keyCode === 13) { // event.key는 "Enter", event.keyCode는 13
		        let projectIdx = ${projectIdx};
		    	let memoTitle = $(this).val();
		    	let nowTime = "${nowMemoTime}";
		    	$(".memo_page_write_date_span").text(nowTime);
		    	$.ajax({
		    		type:'post',
		    		data:{"projectIdx":projectIdx,
		    			"memoTitle":memoTitle},
		    		url:'AjaxUpdateMemoTitleServlet',
		    		success:function(data){
		    			alert(data.memoTitle);
		    		},
		...
		$(".content").keyup(function(event) {
		    ...
		    if (event.key === "Enter" || event.keyCode === 13) { // event.key는 "Enter", event.keyCode는 13
		    	...
		    	$.ajax({
		    		...
		    		url:'AjaxUpdateMemoContentServlet',
		    		success:function(data){
		    			alert(data.memoContent);
		    		},
		    		error:function(r,s,e){alert("error");}
		    	});
		    }
		...
		<div id="main" class="fl">
			...
			<div id="main_content" style="height: 1500px; width: 100%; height: 114px; padding-top: 14px; padding-left: 24px;">
					...
					<div style="max-height: 24px; display: flex; align-items: center; gap: 2px;">
						...
						<div class="small_profile_images_div">
							<div class="list-image">
								...
								<img style="z-index: 3; margin-left: -10px;" class="img-small" src="img\Giljoo.png"> <img style="z-index: 2; margin-left: -10px;" class="img-small" src="img\Hyunwoo.jpg">
							</div>
						</div>
						<!-- 공유버튼 -->
				...
				<div class="file_page_project_name_div">
					...
					<div style="margin-right: 15px; font-size: 20px; display: inline-block; margin-left: 40px;">
						${projectName}</div>
					...
					<div style="display: inline-block; color: grey; font-size: 14px; margin-left: 7px;">○
						상태 설정</div>
				</div>
				<div class="file_page_outline_view_list">
					<span class="over_view_button">
					<svg style="margin-right: 3px; fill: grey;" class="MiniIcon" aria-current="false" viewBox="0 0 24 24" aria-hidden="true" focusable="false">
					...
					<span class="message_view_button">
					<svg style="margin-right: 3px; fill: grey;" class="MiniIcon" aria-current="false" viewBox="0 0 24 24" aria-hidden="true" focusable="false">
					...
					<span class="file_view_button">
					<svg style="margin-right: 3px; fill: grey;" class="MiniIcon" aria-current="true" viewBox="0 0 24 24" aria-hidden="true" focusable="false">
			...
			<div class="main-date">
				...
				<div class="date-text">
					<span>저장됨</span>
					<b>ㆍ</b>
					<span class="memo_page_write_date_span">${lookMemoDto.writeDate}</span>
				</div>
			</div>
			<div class="main" memo_idx="${memoIdx}">
				<div class="main-title">
					<%if(lookMemoDto.getTitle()==null){ %>
						<input type="text" class="title" placeholder="제목 추가" value=""/>
					<%}else{ %>
						<input type="text" class="title" placeholder="제목 추가" value="${lookMemoDto.title}"/>
					<%} %>
				</div>
				<div class="main-content">
					<%if(lookMemoDto.getContent()==null){ %>
						<textarea type="text" class="content" placeholder="내용을 입력하세요"></textarea>
					<%}else{ %>
						<textarea type="text" class="content" placeholder="내용을 입력하세요">${lookMemoDto.content}</textarea>
					<%} %>
				</div>
			</div>
		</div>
