...
<%
	...
	//완료된 작업 보여주기
	ArrayList<WamDto> completeWamList = (ArrayList<WamDto>)request.getAttribute("completeWamList");
	//팀원 리스트 본인제외
...
<head>
<meta charset="UTF-8" />
<title>페이지 - 아사나 홈</title>
<link rel="stylesheet" href="css/Main_Common.css" />
<link rel="stylesheet" href="css/asana_home.css" />
<script src="js/jquery-3.7.1.min.js"></script>
<script>
$(function(){
	/************************* 메인화면 속 프로젝트 클릭시 링크이동 ************************************/
	$(".project-list").click(function(){
		let name = $(this).find("div:nth-child(2) > div:first-child").text().trim();
		if(name==="프로젝트 생성"){
	...
	/*  */
	/************************** 내작업 상태별 리스트 ***************************/
	$(".my_wam_list_header").click(function(){
		$(".my_wam_list_header").removeClass("on");
		$(this).addClass("on");
		let statusName = $(this).text();
		$(".work-under2").removeClass("on");
		$(".work-under1").removeClass("on");
		if(statusName == "예정"){
			$(".work-under2").eq(0).addClass("on");
			$(".work-under1").addClass("on");
		}else if(statusName.startsWith("마감일 지남")){
			$(".work-under2").eq(1).addClass("on");
		}else if(statusName == "완료됨"){
			$(".work-under2").eq(2).addClass("on");
		}
	...
	$(".popup_invite_button").click(function(){
		if($(".popup_invite_button").hasClass("on")){
			let email = "";
			document.querySelectorAll(".input_email_div").forEach(function(item){
	    		email += $(item).text().trim() + "\n";
	    	});
			alert("해당 이메일에게 초대를 발송합니다 : \n" + email);
			$.ajax({
				type: 'post',
				url: 'AjaxMailInviteServlet',
				data: {"email":email,
	...
	/* 사이드바 목록 선택 시 어디로 이동 할지 *//* 사이드바 목록 선택 시 어디로 이동 할지 *//* 사이드바 목록 선택 시 어디로 이동 할지 */
	/* 홈화면 유저 리스트 박스 구성 클릭시 */
	$(".user-list-in").click(function(){
		let memberIdx = $(this).find(".member_idx").text().trim();
	...
	/* 홈화면 내작업 리스트 요소 클릭시 */
	$(".under2-in").click(function(){
		let name = $(this).find("div:first-child > span:nth-child(2)").text().trim();
		alert(name);
	});
	/* 작업 생성 클릭시 */
	$(".home_page_create_new_work").click(function(){
		alert("작업 생성 클릭");
	});
	/* 홈화면 내작업리스트 박스 안의 "프로필이미지" 클릭 시 */
	$(".image-div-in").click(function(){
		location.href="Controller?command=profile_page&member_idx=${loginMemberIdx}";
	});
	/* 홈화면 내작업리스트 박스 안의 "내 작업" 클릭 시  */
	$(".my_work_link_text").click(function(){
		alert("내 작업 리스트로 이동");
	});
	...
	$(".popup_create_project").click(function() {
		location.href="Controller?command=create_project&member_idx=${memberIdx}";
	});
	/* 상단바 팝업창 메시지 생성 클릭 */
		...
		<div id="main">
			<div class="main">
				...
				<div class="main-under">
					<div class="main-under-in">
						<div>
							<div class="main-text-date">${nowDay}</div>
						</div>
						<div>
							<div class="main-hello">안녕하세요, ${loginNickname} 님</div>
						</div>
						<div class="margin1410">
							<div class="circle-gray">
								<div class="circle-in"
									style="border-right: 1px solid lightgray;">
									<select class="select my_period">
										<option>나의 한주</option>
										<option>나의 한달</option>
									</select>
								</div>
								<div class="circle-in">
									<svg class="CheckIcon" viewBox="0 0 32 32" aria-hidden="true"
										focusable="false">
										<path
											d="M28.061 7.939a1.5 1.5 0 0 0-2.121 0L11.001 22.878l-5.939-5.939a1.5 1.5 0 1 0-2.121 2.121l7 7c.293.293.677.439 1.061.439.384 0 .768-.146 1.061-.439l16-16a1.5 1.5 0 0 0 0-2.121h-.002Z"></path></svg>
									<span class="number">${completeWamQuantity}</span> <span>완료된 작업</span>
								</div>
								<div class="circle-in">
									<svg class="CheckIcon" viewBox="0 0 32 32" aria-hidden="true"
										focusable="false">
									...
									<span class="number">${myWorkspaceMemberCount}</span> <span>협업 참여자</span>
								</div>
							</div>
						</div>
						<div class="my-work-div">
							<div class="my-work-top">
								<div class="my-work-top1">
									<div class="image-div">
										<div class="image-div-in">
											<img class="image" src="img/${loginProfileImg}"/>
										</div>
									</div>
									<div class="my-work-top1-in">
										<div class="top1-my-work">
											<div class="center">
												<span class="my_work_link_text">내 작업</span>
												<svg class="MiniIcon15" viewBox="0 0 24 24"
										...
										<div class="status-list">
											<span class="my_wam_list_header on">예정</span>
											<span class="my_wam_list_header">마감일 지남(${wamQuantityAfterDeadline}개)</span>
											<span class="my_wam_list_header">완료됨</span>
										</div>
									</div>
								</div>
							</div>
							<div class="my-work-under">
								<!----------------------------------------- 예정상태 내작업 리스트 ------------------------------------------->
								<div class="work-under1 on">
									<div class="home_page_create_new_work">
										<svg class="MiniIcon15" viewBox="0 0 32 32" aria-hidden="true"
											focusable="false">
											<path
								...
								<div class="work-under2 on">
								<!------------------------------- 작업 생성 추가 div 칸 만드는 코드 --------------------------->
								<%-- 	<%for(int i=0; i<10; i++){ %>
										<div class="under2-in">
											<div>
												<span>
												...
												<span class="under2-text">면접준비하기</span>
											</div>
											<div class="under2-date">
												<div class="status">개발자로...</div>
												<div>금요일</div>
											</div>
										</div>
									<% }%> --%>
									...
									<% for(WamDto dtos : wamDtoList){ %>
										<div class="under2-in">
											<div>
												...
												<span class="under2-text"><%=dtos.getTitle() %></span>
											</div>
											<div class="under2-date">
												<div class="status"><%=dtos.getProjectDto().getName() %></div>
												<div><%=dtos.getDeadlineDate() %></div>
											</div>
										</div>
									<% }%>
								</div>
								...
								<div class="work-under2">
									<% for(WamDto dtos : wamDtoListAfterDeadline){ %>
										<div class="under2-in">
											...
											<div class="under2-date">
												<div class="status"><%=dtos.getProjectDto().getName() %></div>
												<div style="color:red;"><%=dtos.getDeadlineDate() %></div>
											</div>
										</div>
									<%} %>
								</div>
								...
								<!----------------------------------------- 완료됨 리스트 ------------------------------------------->
								<div class="work-under2">
									<%for(WamDto dtos : completeWamList){ %>
										<div class="under2-in">
											<div>
												<span>
													<svg class="profile_page_work_check on" viewBox="0 0 32 32" aria-hidden="true" focusable="false">
								                     <path class="profile_page_work_check_outline on" d="M31,16c0,8.3-6.7,15-15,15S1,24.3,1,16S7.7,1,16,1S31,7.7,31,16z"></path>
								                     <path class="profile_page_work_check_inline on" d="M13.4,22.1c-0.3,0-0.5-0.1-0.7-0.3l-3.9-3.9c-0.4-0.4-0.4-1,0-1.4s1-0.4,1.4,0l3.1,3.1l8.1-8.1c0.4-0.4,1-0.4,1.4,0   s0.4,1,0,1.4l-8.9,8.9C13.9,22,13.7,22.1,13.4,22.1z"></path></svg>
												</span> 
												<span class="under2-text"><%=dtos.getTitle() %></span>
											</div>
											<div class="under2-date">
						...
						<div class="last-under-div">
							<div class="project">
								<div class="project-in">
									<div>
										<span>프로젝트</span> 
										<span> 
											<select style="margin-left: 15px;" class="select project_cetegory">
												<option>최근</option>
												<option>별도 표시</option>
												<option>추천</option>
											</select>
								...
								<div class="project-in-list">
									<div class="project-list">
										<div style="min-width:48px; margin-right: 15px; display: flex; align-items: center; justify-content: center; background-color: rgba(0, 0, 0, 0); width: 48px; height: 48px; border-radius: 10px; border: 1px dashed gray;">
											<svg class="MiniIcon15Plus" viewBox="0 0 32 32" aria-hidden="true" focusable="false">
										...
										<div>
											<div style="color: gray;">프로젝트 생성</div>
											<!-- <div style="font-size:13px; color:gray;">작업 1개 곧 마감</div> -->
										</div>
									</div>
									<!------------------------- 참여프로젝트 리스트 ---------------------------->
									<%for(ProjectDto dtos : pDtoList){ %>
										<div class="project-list" project_idx="<%=dtos.getProjectIdx()%>">
											<div style="min-width:48px; margin-right: 15px; display: flex; align-items: center; justify-content: center; background-color: lightpink; width: 48px; height: 48px; border-radius: 10px;">
							...
							<div class="user">
								<div>
									<div class="project-in">
										<div>
											<span>사용자</span> 
											<span> 
												<select style="margin-left: 15px;" class="select cooperator">
								...
								<div class="user-list">
									...
									<% for(MyWorkspaceDto mwDto : myWorkspaceMemberList){ %>
									<!-- 멤버 -->
									<div class="user-list-in">
										<div>
											<img class="img" src="img/<%=mwDto.getMemberDto().getProfileImg()%>">
										</div>
										<%=mwDto.getMemberDto().getNickname() %>
										<span class="member_idx" style="display:none;"><%=mwDto.getMemberDto().getMemberIdx() %></span>
									</div>
									<%}%>
								</div>
							</div>
