...
public class MainInventoryAction implements Action{
	@Override
	public void execute(HttpServletRequest request, HttpServletResponse response)  throws ServletException, IOException {
		...
		ArrayList<PortfolioAllDto> portfolioAllDto = null;
		try {
			portfolioAllDto = portfolioDao.portfolioTheProjectBelongsTo(projectIdx);
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		try {
			dtolist = wamDao.wamBelongingToTheProject(projectIdx);
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		request.setAttribute("portfolioAllDto", portfolioAllDto);
		request.setAttribute("dtolist", dtolist);
		request.setAttribute("formatter1", formatter1);
		request.setAttribute("formatter2", formatter2);
		request.setAttribute("projectAllDto_getName", projectAllDto.getName());
		...
		request.getRequestDispatcher("main_inventory.jsp?member_idx="+loginMemberIdx+"&project_idx="+projectIdx).forward(request, response);
